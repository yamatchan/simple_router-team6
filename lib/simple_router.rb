...
class SimpleRouter < Trema::Controller
  ...
  def send_later(dpid, message)
    destination_ip = resolve_next_hop(message.destination_ip_address)
...
puts message.sender_hardware_address
    send_flow_mod_add(
      dpid,
      table_id: ARP_LOOKUP_TABLE_ID,
      idle_timeout: 0,
...
#          NiciraRegLoad.new(message.sender_hardware_address, :destination_mac_address),
        ]),
